...
if "`argument'"=="example1"{
	noisily fdrisk, fdr nulllo(log(1/1.1)) nullhi(log(1.1))  stderr(0.8) nullspace(log(1/1.1) log(1.1))  ///
...
if "`argument'"=="example2a"{	
	noisily fdrisk, fdr nulllo(log(1/1.1)) nullhi(log(1.1)) stderr(0.8) nullspace(0) /// 
...
if "`argument'"=="example2b"{
	noisily fdrisk, fdr nulllo(log(1/1.1)) nullhi(log(1.1))  stderr(0.8)  nullspace(0)  alttruncnormal ///
		altspace("2-1*invnorm(1-0.041/2)*0.8" "2+1*invnorm(1-0.041/2)*0.8")  likelihood(0.125)
}

*False discovery risk with LSI and wider null hypothesis:
if "`argument'"=="example3"{
	noisily fdrisk, fdr nulllo(log(1/1.5)) nullhi(log(1.5))  stderr(0.8)  nullspace(0) ///
...
if "`argument'"=="example4"{
	noisily fdrisk, sgpval(1)  nulllo(log(1/1.5)) nullhi(log(1.5))  stderr(0.15) /// 
