...
{p_end}
{synopt:{opt inttype(string)}}  class of interval estimate used for calculating the SGPV. 
{p_end}
{synopt:{opt intlevel(#)}}  level of interval estimate. 
{p_end}
...
{p_end}
{phang}
{opt inttype(string)}     class of interval estimate used for calculating the SGPV. Options are "confidence" for a (1-α)100% confidence interval and "likelihood" for a 1/k likelihood support interval ("credible" not yet supported).
...
{synopt:{cmd:r(power0)}}  probability of SGPV = 0 calculated assuming the parameter is equal to {cmd:true}. That is, {cmd:power.alt} = P(SGPV = 0 | θ = } {cmd:true).  {p_end}
{synopt:{cmd:r(power1)}}  probability of SGPV = 1 calculated assuming the parameter is equal to {cmd:true}. That is, {cmd:power.null} = P(SGPV = 1 | θ = ) {cmd:true). {p_end}
{synopt:{cmd:r(powerinc)}}  probability of 0 < SGPV < 1 calculated assuming the parameter is equal to {cmd:true}. That is, {cmd:power.inc} = P(0 < SGPV < 1 | θ = ) {cmd:true). {p_end}
...
{synopt:{cmd:r(avgI)}}  is the average type I error (unweighted) over the range ({cmd:null.lo}, {cmd:null.hi}). If 0 is included in the null hypothesis region, then "type I error summaries" also contains at 0, the type I error calculated assuming the true parameter value θ is equal to 0. {p_end}
