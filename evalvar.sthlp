...
{dlgtab:Main}
{phang}
{opt com:mand(string)}  the command including options in the evaluated variable should be used 

{phang}
{opt var:id(string)}  specify an alternative variable identifier. To allow more commands, a variable identifier is required to identify the place where the transformed variable should be placed. The default identifier is VAR  

{phang}
{opt egen} use egen instead of gen to create the temporay variable 

{phang}
{opt gen:erate(varname)}  allows to save the transformed variable under the given name. The variable is not allowed to exist before. 

{phang}
{opt rep:lace}  replaced an existing variable with the same an in {opt:generate()}  
...
{synopt:{cmd:r(exp)}}  returns the expression/transformation. {p_end}{synopt:{cmd:r(command)}}  returns the command as it was typed in. {p_end}{synopt:{cmd:r(copt)}}  {p_end}
