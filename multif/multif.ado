...
version 10.0
	syntax varlist(min=2),  CONDition(string asis) CONnection(string) [COMmand(string) COMOPTion(string) VARid(string) ADDif(string) ADDCon(string) test DISPlay] 
	*Parse syntax
	local vars `varlist'
	if "`varid'"=="" local varid VAR
	*Check if varid is correctly set in condition -> not correctly working yet.									  
...
*Test if all variable identifiers are the same.
*Does not work yet for all cases
*If there is more than one condition in the condition() option, then returns incorrect result!

program define ParseCondError
	syntax,  cond(string asis) varid(string)
	local 0  `"`cond'"'
	/*
