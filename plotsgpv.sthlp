...
{syntab:Colors}
{synopt:{opt nullc:ol(string)}}  coloring of the null interval (indifference zone). Default is the R-color Hawkes Blue.
{p_end}
...
{p_end}
{synopt:{opt x:show(string)}}  a number representing the maximum ranking on the x-axis that is displayed. Default is to display all intervals.
{p_end}
...
You can set the color to any other available color in Stata. See {help:colorstyle} for more information.
...
You can see the color before plotting via:
...
{pstd}		
You can set the colors to any other available color in Stata. See {help:colorstyle} for more information. You need to set three colors or else you receive an error message. {p_end}	
{phang}
{opt intcol1(string asis)}  color of the interval with the R-color "firebrick3" as the default for SGPV = {it:0}

{phang}
{opt intcol2(string asis)}  ccolor of the interval with the R-color "cornflowerblue" as the default for {it:0} < SGPV < {it:1}
...
{phang}
{opt x:show(string)}    a number representing the maximum ranking on the x-axis that is displayed. Default is to display all intervals.

{phang}
{opt nullpt(#)} 	a number representing a point null hypothesis. Default is 0. If set, the command will draw a horizontal dashed red line at this location.
...
{bf:Replicate the example plot from the R-code with the example dataset provided with this command:}
	{stata sysuse leukstats}
	plotsgpv, esthi(ci_hi) estlo(ci_lo) nulllo(-0.3) nullhi(0.3) nomata replace noshow setorder(p_value) title("Leukemia Example") ///
	xtitle("Classical p-value ranking") ytitle("Fold Change (base 10)") ///
